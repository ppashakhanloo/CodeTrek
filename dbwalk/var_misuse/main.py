...
if __name__ == '__main__':
    ...
    if args.phase == 'eval':
        ...
        model.load_state_dict(torch.load(model_dump, map_location=device))
        db_eval = db_class(args, prog_dict, args.data_dir, 'eval')
        eval_loader = db_eval.get_test_loader(args)
        binary_eval_dataset(model, 'eval', eval_loader, device)
        sys.exit()

    db_train = db_class(args, prog_dict, args.data_dir, 'train', sample_prob=[0.5, 0.5], shuffle_var=args.shuffle_var)
    ...
    if args.num_train_proc > 1:
        ...
        for rank, device in enumerate(devices):
            proc = mp.Process(target=train_mp, args=(args, rank, device, prog_dict, model, db_train, db_dev, binary_eval_dataset))
            proc.start()
        for proc in procs:
            proc.join()
    else:
