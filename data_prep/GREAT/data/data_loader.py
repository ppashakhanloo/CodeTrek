...
class DataLoader():
  ...
  def to_sample(self, json_data):
    ...
    len_tokens = len(json_data["source_tokens"])
    tokens = [self.vocabulary.translate(t)[:self.config["max_token_length"]] for t in ["#NEWLINE#","#NEWLINE#"]+json_data["source_tokens"]]
    edges = parse_edges(json_data["edges"])
    label = 0 if json_data["label"] == 'used' else 1
    return (tokens, edges, label, [], [])
  ...
  def to_batch(self, sample_generator, mode):
    ...
    def make_batch(buffer):
      ...
      items1 = tf.cast(tf.concat(batch[3], axis=0), 'int32')  # Explicitly cast to int32 since repair targets can be empty, in which case TF defaults to float32
      ...
      return buffer, (token_tensor, edge_tensor, label, items1, items2)
