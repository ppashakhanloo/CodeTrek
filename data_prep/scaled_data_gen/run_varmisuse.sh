...
pred_kind="$5" # prog_cls, loc_cls, loc_rep
...
do
  ...
  screen -S $f -d -m python varmisuse_graphgen.py $file_path $bucket tables "$walks_or_graphs"_"$pred_kind" $home_path $walks_or_graphs $pred_kind
  echo $f $?
done
