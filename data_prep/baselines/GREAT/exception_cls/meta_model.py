...
class ExceptionClsModel(tf.keras.layers.Layer):
  def __init__(self, config, vocab_dim):
    super(ExceptionClsModel, self).__init__()
    self.config = config
  ...
  def build(self, _):
    ...
    self.prediction = tf.keras.layers.Dense(20)
