...
from data import data_loader_varmisuse_loc, vocabulary
from meta_model import VarmisuseLocModel
...
def main():
  ...
  data = data_loader_defuse_loc.DataLoader(args.data_path, config["data"], vocabulary.Vocabulary(args.vocabulary_path))
  if args.eval_only:
...
def test(data, config, model_path, log_path):
  model = VarmisuseLocModel(config['model'], data.vocabulary.vocab_dim)
  ...
  while tracker.ckpt.step < config["training"]["max_steps"]:
    for batch in data.batcher(mode='train'):
      ...
      if counter % config['data']['valid_interval'] == 0:
        avg_accs, _ = evaluate(data, config, model)
