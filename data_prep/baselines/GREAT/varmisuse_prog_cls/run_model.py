...
from data import data_loader_varmisuse_prog, vocabulary
from meta_model import VarmisuseProgModel
...
def main():
  ...
  data = data_loader_varmisuse_prog.DataLoader(args.data_path, config["data"], vocabulary.Vocabulary(args.vocabulary_path))
  if args.eval_only:
...
def test(data, config, model_path, log_path):
  model = VarmisuseProgModel(config['model'], data.vocabulary.vocab_dim)
