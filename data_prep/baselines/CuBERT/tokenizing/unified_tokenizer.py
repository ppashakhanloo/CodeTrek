...
def wrap_subtoken_list(st_list):
  new_list = []
  for st in st_list:
    new_list.append("\'"+st+"\'")
  return new_list
...
def code_to_cubert_sentences(
  ...
  sentences = [s + [NEWLINE] for s in raw_sentences
              ]  # type: List[List[Text]]
  ...
  for sentence in sentences:
    ...
    decoded_tokens = flattened_encodings  #subword_tokenizer.decode_list(
...
def printable_list(vocab_list):
  printable_list = []
  e = sys.stdout.encoding
  errors = 'ignore'
  for item in vocab_list:
    printable_list.append(str(item.encode(e, errors).decode(e, errors)))
