...
class InputExample(object):
  def __init__(self, guid, function, label=None, location=None):
    self.guid = guid
  ...
  def _read_json(cls, input_file, quotechar=None):
    with tf.compat.v1.gfile.GFile(input_file) as f:
      ...
      for line in lines:
        ...
        data_lines.append((jline['function'], jline['label'], jline['location']))

      return data_lines

class DefuseLocProcessor(DataProcessor):
  def get_train_examples(self, data_dir):
    data_tuples = []
    ...
    for (i, data_item) in enumerate(data_tuples):
      ...
      examples.append(InputExample(guid=guid, function=data_item[0], label=label, location=data_item[2]))
    return examples
...
def convert_single_example(index, example, label_list, seq_length, tokenizer):
  ...
  function_int = []
  for item in example.function:
    if item not in tokenizer:
      function_int.append(len(tokenizer))
    else:
      function_int.append(tokenizer[item])

  if len(function_int) > seq_length:
    function_int = function_int[0:seq_length]

  segment_ids = example.location
  if len(segment_ids) > seq_length:
    segment_ids = segment_ids[:seq_length]
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    print(" ~~~~ INDEX : ", ex_index, "/", len(examples))
