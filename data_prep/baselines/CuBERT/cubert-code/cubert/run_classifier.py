...
import tensorflow as tf #Tensorflow 1.15.2
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  #all_tokens = "[CLS]_" + tokens_a + "[SEP]_" + tokens_b
  #assert example.text_b is None

  # Account for [CLS] and [SEP] with "- 2"
  #if len(token_a + token_b) > max_seq_length:
    # = (tokens_a + token_b)[0:(max_seq_length - 2)]
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    print(f"num_actual_predict_examples:{num_actual_predict_examples}")
    if FLAGS.use_tpu:
      # TPU requires a fixed batch size for all batches, therefore the number
      # of examples must be a multiple of the batch size, or else examples
      # will get dropped. So we pad with fake examples which are ignored
